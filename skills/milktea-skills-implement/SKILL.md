...
讀取 Spec 的「使用情境 QA」開關；未啟用或沒有此欄位時，跳過收尾 QA。Spec 引用的既有 QA 問題，依本次核准範圍處理，不掃描其他 QA 文件。
...
本次啟用使用情境 QA 時，在全部 Tickets 與既有 Agent 驗收完成後、產出完成報告前，依派工契約委派一個新的隔離 QA Agent，載入 `milktea-skills-usage-scenario-qa`。QA Agent 必須能實際操作要測試的產品。例如測試網頁時，要能開啟頁面、點擊與輸入。無法操作的部分，應清楚回報沒有測到，不能當作已通過。

本階段 QA 的所有發現，包括紅燈問題，都只記錄與回報，不交回 Developer、不重開 Ticket，也不進入本輪修正或共識迴圈；留待後續 Grill-me 決定是否採納。報告須分別呈現原核准範圍的驗收結果與收尾 QA 發現，不得因原驗收通過而省略 QA 問題。

Coordinator 負責保存 QA 問題，每題一份：`docs/qa/pending/<工作名稱>-qa-<序號>.md`。以檔名作為固定問題 ID，記錄來源工作、測試版本、QA 回報的情境／證據／影響／燈號／改善方向，以及處理狀態「待決定」。只在有發現時建立問題文件，不建立空目錄或湊數項目。

本次 Spec 已採納的既有 QA 問題，只有在原問題的改善結果已實際驗證後，才由 Coordinator 補上解決證據、標記「已解決」，並從 active 移到 archived；未完成或無法驗證時留在 active。移動時保留 ID 與檔名，更新已知的 Spec 與完成報告引用。
...
- 使用情境 QA 是否啟用；啟用時列出實際檢查範圍、結果、驗證限制與問題文件連結。零發現時記錄「本次未發現問題」。
...
- 適用的使用者操作流程已實際驗證。
- 最終 HTML 報告已產生並可直接開啟。
- 若本次啟用使用情境 QA，須回報實際測了什麼、發現哪些問題，以及哪些部分沒有測到。新發現的問題由 Coordinator 寫入 `docs/qa/pending/`，留待後續 Grill-me 討論，不在本輪自動修正。
