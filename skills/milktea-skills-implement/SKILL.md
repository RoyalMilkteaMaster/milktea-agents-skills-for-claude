...
本次啟用使用情境 QA 時，在全部 Tickets 與既有 Agent 驗收完成後、產出完成報告前，依派工契約委派一個新的隔離 QA Agent，載入 `milktea-skills-usage-scenario-qa`。選用能實際操作本次產品入口的執行方式；無法取得必要操作能力時，記錄未能驗證的範圍，不宣稱完成 QA。
...
- 適用的使用者操作流程已實際驗證。
- 最終 HTML 報告已產生並可直接開啟。
- 若啟用使用情境 QA，已完成可驗證範圍並記錄結果與限制；新發現已交接至 pending，不要求在本輪修正。
