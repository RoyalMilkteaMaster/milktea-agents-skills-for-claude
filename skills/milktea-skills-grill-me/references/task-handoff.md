...
只在 Tickets 核准、`/milktea-agents-skills-for-claude:milktea-skills-to-ticket` 已產生完整交接內容，且使用者選擇建立新的執行 Task 後讀取。交接內容是唯一 Task prompt；不得改寫或產生另一版本。
...
3. 將 `/milktea-agents-skills-for-claude:milktea-skills-to-ticket` 產生並已顯示的完整交接內容原樣作為新 Task prompt。
